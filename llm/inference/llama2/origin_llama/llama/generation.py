...
class Llama:
    ...
    def generate(
        ...
        if logprobs:
            token_logprobs = ops.zeros_like(tokens, dtype=mindspore.float32)

        prev_pos = 0
        eos_reached = mindspore.Tensor([False] * bsz)
        input_text_mask = tokens != pad_id
        if min_prompt_len == total_len:
            logits = self.model(tokens, prev_pos)
            token_logprobs = -ops.cross_entropy(
        ...
        for cur_pos in range(min_prompt_len, total_len):
            ...
            if temperature > 0:
                probs = ops.softmax(logits[:, -1] / temperature, axis=-1)
            ...
            if logprobs:
                token_logprobs[:, prev_pos + 1 : cur_pos + 1] = -ops.cross_entropy(
                    input=logits.transpose(1, 2),
...
def sample_top_p(probs, p):
    ...
    Args:
        probs (mindspore.Tensor): Probability distribution tensor.
        p (float): Probability threshold for top-p sampling.

    Returns:
        mindspore.Tensor: Sampled token indices.
