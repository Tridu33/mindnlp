...
from ..nn.functional import pad, softplus
...
class ReshapeTransform(Transform):
    ...
    elements, just as for :meth:`mindspore.Tensor.reshape`.

    Arguments:
        in_shape: The input event shape.
        out_shape: The output event shape.
    """
...
class PowerTransform(Transform):
    ...
    def forward_shape(self, shape):
        return ops.broadcast_shapes(shape, getattr(self.exponent, "shape", ()))
...
def _clipped_sigmoid(x):
    finfo = ops.finfo(x.dtype)
    return ops.clamp(ops.sigmoid(x), min=finfo.tiny, max=1.0 - finfo.eps)
...
class CorrCholeskyTransform(Transform):
    ...
    def _call(self, x):
        ...
        r = r + ops.eye(r.shape[-1], dtype=r.dtype)
    ...
    def _inverse(self, y):
        ...
        y_cumsum = 1 - ops.cumsum(y * y, dim=-1)
...
class StickBreakingTransform(Transform):
    ...
    def _inverse(self, y):
        ...
        sf = ops.clamp(sf, min=ops.finfo(y.dtype).tiny)
...
class CatTransform(Transform):
    ...
    in a way compatible with :func:`ops.cat`.

    Example::

       x0 = ops.cat([ops.range(1, 10), ops.range(1, 10)], dim=0)
       x = ops.cat([x0, x0], dim=0)
    ...
    def _call(self, x):
        ...
        return ops.cat(yslices, dim=self.dim)
    ...
    def _inverse(self, y):
        ...
        for trans, length in zip(self.transforms, self.lengths):
            ...
            start = start + length  # avoid += for jit compat
        return ops.cat(xslices, dim=self.dim)
    ...
    def log_abs_det_jacobian(self, x, y):
        ...
        if dim < 0:
            return ops.cat(logdetjacs, dim=dim)
        else:
...
class StackTransform(Transform):
    ...
    in a way compatible with :func:`ops.stack`.

    Example::

       x = ops.stack([ops.range(1, 10), ops.range(1, 10)], dim=1)
    ...
    def _call(self, x):
        ...
        for xslice, trans in zip(self._slice(x), self.transforms):
            yslices.append(trans(xslice))
        return ops.stack(yslices, dim=self.dim)
    ...
    def _inverse(self, y):
        ...
        for yslice, trans in zip(self._slice(y), self.transforms):
            xslices.append(trans.inv(yslice))
        return ops.stack(xslices, dim=self.dim)
    ...
    def log_abs_det_jacobian(self, x, y):
        ...
        return ops.stack(logdetjacs, dim=self.dim)
