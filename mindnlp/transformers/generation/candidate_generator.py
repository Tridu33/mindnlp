...
class CandidateGenerator:
    ...
    def get_candidates(self, input_ids: mindspore.Tensor) -> Tuple[mindspore.Tensor, Optional[mindspore.Tensor]]:
        """
        ...
        Return:
            `mindspore.Tensor` of shape `(batch_size, candidate_length)` containing the candidate sequences to be
            assessed by the model and, optionally, a `mindspore.Tensor` of shape `(batch_size, candidate_length,
    ...
    def update_candidate_strategy(self, input_ids: mindspore.Tensor, scores: mindspore.Tensor, num_matches: int):
        """
        ...
        Args:
            input_ids (`mindspore.Tensor` of shape `(batch_size, sequence_length)`):
                Indices of input sequence tokens in the vocabulary. [What are input IDs?](../glossary#input-ids)
            scores (`mindspore.Tensor` of shape `(batch_size, candidate_length, config.vocab_size)`):
...
class AssistedCandidateGenerator(CandidateGenerator):
    ...
    Args:
        ...
        model_kwargs (`Dict`):
            ...
            model as well.
        inputs_tensor (`mindspore.Tensor`, *optional*):
    ...
    def __init__(
        ...
        model_kwargs: Dict,
        inputs_tensor: Optional[mindspore.Tensor] = None,
        ...
        for key, value in model_kwargs.items():  # deepcopy crashes if we attempt to copy encoder outputs with grads
            if key not in ("encoder_outputs", "assistant_encoder_outputs", "past_key_values"):
                assistant_kwargs[key] = (
                    value if isinstance(value, mindspore.Tensor) else copy.deepcopy(value)
                )
        ...
        if assistant_model.config.is_encoder_decoder:
            ...
            self.assistant_kwargs["attention_mask"] = self.assistant_kwargs.get(
                ...
                ops.ones((input_ids.shape[0], 1), dtype=mindspore.int64),
            )
        else:
    ...
    def get_candidates(self, input_ids: mindspore.Tensor) -> Tuple[mindspore.Tensor, Optional[mindspore.Tensor]]:
        ...
        Return:
            `mindspore.Tensor` of shape `(batch_size, candidate_length)` containing the candidate sequences to be
            assessed by the model and a `mindspore.Tensor` of shape `(batch_size, candidate_length,
            vocabulary_size)` containing the logits associated to each candidate.
        """
        ...
        candidate_logits = ops.stack(assistant_output.scores, dim=1)
...
class PromptLookupCandidateGenerator(CandidateGenerator):
    ...
    def __init__(
        self,
        eos_token_id: mindspore.Tensor = None,
    ...
    def get_candidates(self, input_ids: mindspore.Tensor) -> Tuple[mindspore.Tensor, Optional[mindspore.Tensor]]:
        ...
        Return:
            `mindspore.Tensor` of shape `(num_candidates, candidate_length)`: The candidate sequences to be tried.
        """
        ...
        for ngram_size in range(min(self.max_matching_ngram_size, input_length - 1), 0, -1):
            ...
            windows = ops.unfold(input_ids, dimension=1, size=ngram_size, step=1)
            ...
            match_indices = ops.nonzero(matches, as_tuple=True)[1]
            ...
            for idx in match_indices:
                ...
                if start_idx < end_idx:
                    ...
                    mask = ops.isin(chosen_ids, self.eos_token_id)
                    match_indices_eos = ops.nonzero(mask)
        ...
        candidate_input_ids = ops.cat((input_ids, chosen_ids), dim=1)
