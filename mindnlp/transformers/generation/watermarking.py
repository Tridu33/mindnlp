...
class WatermarkDetector:
    ...
    def _get_ngram_score(self, prefix: mindspore.Tensor, target: int):
        greenlist_ids = self.processor._get_greenlist_ids(prefix)
        return target in greenlist_ids

    def _score_ngrams_in_passage(self, input_ids: mindspore.Tensor):
        batch_size, seq_length = input_ids.shape
        selfhash = int(self.processor.seeding_scheme == "selfhash")
        n = self.processor.context_width + 1 - selfhash
        indices = ops.arange(n).unsqueeze(0) + ops.arange(seq_length - n + 1).unsqueeze(1)
