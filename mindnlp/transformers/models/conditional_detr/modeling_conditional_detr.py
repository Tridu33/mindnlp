...
def replace_batch_norm(model: nn.Module):
    r"""
    Recursively replace all `ops.nn.BatchNorm2d` with `ConditionalDetrFrozenBatchNorm2d`.
    ...
    for name, cell in model.cells_and_names():
        ...
        if isinstance(cell, nn.BatchNorm2d):
            new_cell = ConditionalDetrFrozenBatchNorm2d(cell.num_features)

            new_cell.weight.assign_value(cell.weight)
            new_cell.bias.assign_value(cell.bias)
            new_cell.running_mean.assign_value(cell.running_mean)
            new_cell.running_var.assign_value(cell.running_var)
...
def gen_sine_position_embeddings(pos_tensor, d_model):
    scale = 2 * math.pi
    dim = d_model // 2
    dim_t = ops.arange(dim, dtype=mindspore.float32)
    dim_t = 10000 ** (2 * ops.div(dim_t, 2, rounding_mode="floor") / dim)
...
class ConditionalDetrAttention(nn.Module):
    ...
    def _qk_shape(self, tensor: mindspore.Tensor, seq_len: int, batch_size: int):
        return tensor.view(batch_size, seq_len, self.num_heads, self.head_dim).swapaxes(1, 2)

    def _v_shape(self, tensor: mindspore.Tensor, seq_len: int, batch_size: int):
        return tensor.view(batch_size, seq_len, self.num_heads, self.v_head_dim).swapaxes(1, 2)
...
class ConditionalDetrModel(ConditionalDetrPreTrainedModel):
    ...
    ) -> Union[Tuple[mindspore.Tensor], ConditionalDetrModelOutput]:
        r"""
        Returns:
...
class ConditionalDetrForObjectDetection(ConditionalDetrPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[mindspore.Tensor], ConditionalDetrObjectDetectionOutput]:
        r"""
        ...
        reference_before_sigmoid = inverse_sigmoid(reference).swapaxes(0, 1)
        outputs_coords = []
        hs = sequence_output
...
class ConditionalDetrForSegmentation(ConditionalDetrPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[mindspore.Tensor], ConditionalDetrSegmentationOutput]:
        r"""
        labels (`List[Dict]` of len `(batch_size,)`, *optional*):
