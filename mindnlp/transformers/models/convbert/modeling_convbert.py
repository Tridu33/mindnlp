...
class ConvBertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            ops.initialize(module.weight, 'zeros')
            ops.initialize(module.bias, 'ones')
...
class ConvBertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        mixed_key_conv_attn_layer = self.key_conv_attn_layer(hidden_states.swapaxes(1, 2))
        ...
        conv_kernel_layer = ops.reshape(conv_kernel_layer, [-1, self.conv_kernel_size, 1])
        conv_kernel_layer = ops.softmax(conv_kernel_layer, dim=1)

        conv_out_layer = self.conv_out_layer(hidden_states)
        conv_out_layer = ops.reshape(conv_out_layer, [batch_size, -1, self.all_head_size])
        ...
        conv_out_layer = ops.reshape(conv_out_layer, [-1, self.attention_head_size, self.conv_kernel_size])
        ...
        conv_out = ops.reshape(conv_out_layer, [batch_size, -1, self.num_attention_heads, self.attention_head_size])
...
class GroupedLinearLayer(nn.Module):
    def __init__(self, input_size, output_size, num_groups):
        ...
        self.weight = Parameter(ops.zeros((self.num_groups, self.group_in_dim, self.group_out_dim)))
...
class ConvBertGeneratorPredictions(nn.Module):
    ...
    def forward(self, generator_hidden_states: mindspore.Tensor) -> mindspore.Tensor:
...
class ConvBertClassificationHead(nn.Module):
    ...
    def forward(self, hidden_states: mindspore.Tensor, **kwargs) -> mindspore.Tensor:
