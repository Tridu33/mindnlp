...
class DbrxSdpaAttention(DbrxAttention):
    """
    Dbrx attention module using F.scaled_dot_product_attention. This module inherits from
    ...
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "DbrxModel is using DbrxSdpaAttention, but `F.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
class DbrxFFN(nn.Module):
    ...
    def forward(self, x: mindspore.Tensor) -> Tuple[mindspore.Tensor, mindspore.Tensor]:
...
class DbrxBlock(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        Tuple[mindspore.Tensor, Optional[Cache]],
        Tuple[mindspore.Tensor, Optional[mindspore.Tensor], Optional[Cache]],
        ...
        Args:
            ...
            position_ids (`mindspore.Tensor`): position ids of shape `(batch, seq_len)`
            ...
            cache_position (`mindspore.Tensor`, optional): position ids of the cache
        """
