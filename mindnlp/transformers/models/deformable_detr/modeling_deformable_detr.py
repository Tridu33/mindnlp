...
"""MindSpore Deformable DETR model."""
...
class DeformableDetrPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if hasattr(module, "reference_points") and not self.config.two_stage:
            ...
            nn.init.zeros_(module.reference_points.bias)
    ...
    def gen_encoder_output_proposals(self, enc_output, padding_mask, spatial_shapes):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ...
            grid_y, grid_x = meshgrid(
                ops.linspace(0, height - 1, height, dtype=enc_output.dtype),
                ops.linspace(0, width - 1, width, dtype=enc_output.dtype),
                indexing="ij",
            )
            ...
            grid = (
                grid.unsqueeze(0).broadcast_to((batch_size, -1, -1, -1)) + 0.5
            ) / scale
    ...
    def forward(
        ...
        if self.config.two_stage:
            ...
            topk_coords_logits = ops.gather(
                ...
                1,
                topk_proposals.unsqueeze(-1).tile((1, 1, 4)),
            )
...
class DeformableDetrForObjectDetection(DeformableDetrPreTrainedModel):
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        if config.with_box_refine:
            ...
            self.bbox_embed[0].layers[-1].bias[2:] = -2.0

            # hack implementation for iterative bounding box refinement
            self.model.decoder.bbox_embed = self.bbox_embed
        else:
            self.bbox_embed.layers[-1].bias[2:] = -2.0
    ...
    def forward(self, outputs, targets):
        ...
        if "enc_outputs" in outputs:
            ...
            for bt in bin_targets:
                bt["class_labels"] = ops.zeros_like(bt["class_labels"])
...
def _upcast(t: mindspore.Tensor) -> mindspore.Tensor:
...
class NestedTensor:
    def __init__(self, tensors, mask: Optional[mindspore.Tensor]):
...
def nested_tensor_from_tensor_list(tensor_list: List[mindspore.Tensor]):
