...
"""MindNLP Flaubert model, based on XLM."""
import itertools
import math
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input, mask, kv=None, cache=None, head_mask=None, output_attentions=False):
        ...
        def shape(x):
            """projection"""
            return ops.transpose(x.view(bs, -1, self.n_heads, dim_per_head), 1, 2)

        def unshape(x):
            """compute context"""
            return ops.transpose(x, 1, 2).view(bs, -1, self.n_heads * dim_per_head)
        ...
        scores = ops.matmul(q, ops.transpose(k, 2, 3))  # (bs, n_heads, qlen, klen)
        mask = (mask == 0).view(mask_reshape).expand_as(scores)  # (bs, n_heads, qlen, klen)
        scores = ops.masked_fill(scores, mask, ops.finfo(scores.dtype).min)  # (bs, n_heads, qlen, klen)
...
FLAUBERT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        lengths (`mindspore.Tensor` of shape `(batch_size,)`, *optional*):
            Length of each sentence that can be used to avoid performing attention on padding token indices. You can
            also use `attention_mask` for the same result (see above), kept here for compatibility. Indices selected in
            `[0, ..., input_ids.shape[-1]`:
        cache (`Dict[str, mindspore.Tensor]`, *optional*):
            Dictionary strings to `mindspore.Tensor` that contains precomputed hidden-states (key and values in the
...
# Copied transformers.models.xlm.modeling_xlm.
# XLMForSequenceClassification with XLM_INPUTS->FLAUBERT_INPUTS,XLM->Flaubert
...
# Copied from transformers.models.xlm.modeling_xlm.
# XLMForTokenClassification with XLM_INPUTS->FLAUBERT_INPUTS,XLM->Flaubert
...
# Copied from transformers.models.xlm.modeling_xlm.
# XLMForQuestionAnsweringSimple with XLM_INPUTS->FLAUBERT_INPUTS,XLM->Flaubert
...
class FlaubertForQuestionAnswering(FlaubertPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from mindnlp.transformers import XLMTokenizer, XLMForQuestionAnswering
        >>> import mindspore
