...
class FocalNetModulation(nn.Module):
    ...
    def forward(self, hidden_state):
        ...
        x = self.projection_in(hidden_state).permute(0, 3, 1, 2)
        q, ctx, self.gates = ops.split(x, (num_channels, num_channels, self.focal_level + 1), 1)
        ...
        ctx_global = self.activation(ctx.mean(2, keep_dims=True).mean(3, keep_dims=True))
...
__all__ = ['FocalNetEmbeddings','FocalNetPatchEmbeddings','FocalNetDropPath',
           'FocalNetModulation','FocalNetMlp','FocalNetLayer','FocalNetStage',
           'FocalNetEncoder','FocalNetPreTrainedModel','FocalNetModel',
           'FocalNetForMaskedImageModeling','FocalNetForImageClassification',
           'FocalNetBackbone']
