...
class LayoutLMv3TextEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # position_ids (1, len position emb) is in memory and exported when serialized
    ...
    def calculate_spatial_position_embeddings(self, bbox):
        ...
        # below is the difference between LayoutLMEmbeddingsV2 (ops.cat) and LayoutLMEmbeddingsV1 (add)
...
class LayoutLMv3SelfAttention(nn.Module):
    ...
    def cogview_attention(self, attention_scores, alpha=32):
        ...
        will result in a slower speed and a little bias. Can use ops.allclose(standard_attention_probs,
        cogview_attention_probs, atol=1e-08) for comparison. The smaller atol (e.g., 1e-08), the better.
        """
        scaled_attention_scores = attention_scores / alpha
        max_value = scaled_attention_scores.amax(-1).unsqueeze(-1)
    ...
    def forward(
        ...
        attention_scores = ops.matmul(query_layer / math.sqrt(self.attention_head_size), ops.transpose(key_layer, -1, -2))
...
class LayoutLMv3Model(LayoutLMv3PreTrainedModel):
    def __init__(self, config):
        ...
        if config.visual_embed:
            ...
            self.pos_embed = nn.Parameter(ops.zeros(1, size * size + 1, config.hidden_size))
    ...
    def init_visual_bbox(self, image_size=(14, 14), max_len=1000):
        ...
        visual_bbox = ops.stack(
            [
                visual_bbox_x[:-1].tile((image_size[0], 1)),
                ops.transpose(visual_bbox_y[:-1].tile((image_size[1], 1)), 0, 1),
                visual_bbox_x[1:].tile((image_size[0], 1)),
                ops.transpose(visual_bbox_y[1:].tile((image_size[1], 1)), 0, 1),
            ],
            dim=-1,
    ...
    def calculate_visual_bbox(self, dtype, batch_size):
        ...
        visual_bbox = visual_bbox.to(dtype)
        return visual_bbox
    ...
    def forward(
        ...
        if pixel_values is not None:
            ...
            visual_attention_mask = ops.ones(
                (batch_size, visual_embeddings.shape[1]), dtype=mindspore.int64
            )
            if attention_mask is not None:
                attention_mask = ops.cat([attention_mask, visual_attention_mask], dim=1)
            else:
                attention_mask = visual_attention_mask

            if self.config.has_relative_attention_bias or self.config.has_spatial_attention_bias:
                if self.config.has_spatial_attention_bias:
                    visual_bbox = self.calculate_visual_bbox(dtype=mindspore.int64, batch_size=batch_size)
                    if bbox is not None:
                        final_bbox = ops.cat([bbox, visual_bbox], dim=1)
                    else:
                        final_bbox = visual_bbox

                visual_position_ids = ops.arange(
                    0, visual_embeddings.shape[1], dtype=mindspore.int64
                ).tile((batch_size, 1))
                if input_ids is not None or inputs_embeds is not None:
                    position_ids = ops.arange(0, input_shape[1]).unsqueeze(0)
            ...
            if input_ids is not None or inputs_embeds is not None:
                embedding_output = ops.cat([embedding_output, visual_embeddings], dim=1)
            else:
