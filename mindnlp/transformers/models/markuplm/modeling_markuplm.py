...
class XPathEmbeddings(nn.Module):
    ...
    def forward(self, xpath_tags_seq=None, xpath_subs_seq=None):
        ...
        xpath_tags_embeddings = ops.cat(xpath_tags_embeddings, dim=-1)
        xpath_subs_embeddings = ops.cat(xpath_subs_embeddings, dim=-1)
...
class MarkupLMEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if xpath_tags_seq is None:
            xpath_tags_seq = self.config.tag_pad_id * ops.ones(
                tuple(list(input_shape) + [self.max_depth]), dtype=mindspore.int64
            )
        if xpath_subs_seq is None:
            xpath_subs_seq = self.config.subs_pad_id * ops.ones(
                tuple(list(input_shape) + [self.max_depth]), dtype=mindspore.int64
            )
...
class MarkupLMModel(MarkupLMPreTrainedModel):
    ...
    def forward(
        ...
        xpath_subs_seq: Optional[mindspore.Tensor] = None,
        attention_mask: Optional[mindspore.Tensor] = None,
