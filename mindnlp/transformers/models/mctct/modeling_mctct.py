...
class MCTCTConv1dSubsampler(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(p = config.conv_dropout)
    ...
    def forward(self, input_features):
        ...
        input_features = ops.pad(input_features, (0, 0, padding, padding), "constant", 0)
...
class MCTCTSelfAttention(nn.Module):
    ...
    def relative_position_embedding_rotate(self, scores):
        ...
        scores = scores[:, halfpoint : halfpoint + seq_len].swapaxes(1, 2)  # e.g. [10, 14, 14, 4]
    ...
    def forward(
        ...
        relative_position_scores = ops.einsum("lh, bche -> bcle", positional_embedding, query_layer.swapaxes(2, 3))
...
class MCTCTPreTrainedModel(PreTrainedModel):
    ...
    def _get_feat_extract_output_lengths(self, input_lengths: ms.Tensor):
        ...
        for _, kernel_sz, stride in zip(
            ...
            #TODO
            # input_lengths = ops.div(input_lengths, stride, rounding_mode="trunc") + 1
    ...
    def _get_feature_vector_attention_mask(self, feature_vector_length, attention_mask):
        ...
        t = attention_mask.sum(-1).to(ms.int32)
        subsampled_lengths = self._get_feat_extract_output_lengths(t)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            attention_mask = (
                attention_mask
                if attention_mask is not None
                else ops.ones(input_features.shape[:-1], dtype=ms.int64)
            )
...
__all__ = ['MCTCTLayer','MCTCTPreTrainedModel','MCTCTEncoder','MCTCTModel','MCTCTForCTC']
