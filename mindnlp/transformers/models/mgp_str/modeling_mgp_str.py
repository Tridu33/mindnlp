...
class MgpstrModelOutput(ModelOutput):
    ...
    Args:
        logits (`tuple(ms.Tensor)` of shape `(batch_size, config.num_character_labels)`):
            Tuple of `ms.Tensor` (one for the output of character of shape `(batch_size,
        ...
        attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, config.max_token_length,
            sequence_length, sequence_length)`.

            Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
            heads.
        a3_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_a3_attentions=True` is passed or when `config.output_a3_attentions=True`):
            Tuple of `ms.Tensor` (one for the attention of character, + one for the attention of bpe`, + one
...
class MgpstrForSceneTextRecognition(MgpstrPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[ms.Tensor], MgpstrModelOutput]:
        r"""
