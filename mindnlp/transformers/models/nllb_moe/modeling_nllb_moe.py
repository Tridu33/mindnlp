...
def create_position_ids_from_input_ids(
    ...
    incremental_indices = (
        ops.cumsum(mask, dim=1).type_as(mask) + past_key_values_length
    ) * mask
    return incremental_indices.long() + padding_idx


def load_balancing_loss_func(
    router_probs: mindspore.Tensor, expert_indices: mindspore.Tensor
) -> float:
    r"""
    ...
    return ops.mean(tokens_per_group_and_expert * router_prob_per_group_and_expert) * (
        num_experts**2
    )
...
class NllbMoeSinusoidalPositionalEmbedding(nn.Module):
    ...
    def get_embedding(
        ...
        emb = ops.arange(num_embeddings, dtype=mindspore.int64).float().unsqueeze(
            1
        ) * emb.unsqueeze(0)
    ...
    def create_position_ids_from_inputs_embeds(
        ...
        return (
            position_ids.unsqueeze(0).broadcast_to((input_shape))
...
class NllbMoeTop2Router(nn.Module):
    ...
    def route_tokens(
        ...
        top_1_expert_index = ops.argmax(router_probs, dim=-1)
        ...
        top_2_expert_index = ops.argmax(logits_except_top_1, dim=-1)
        top_2_mask = F.one_hot(top_2_expert_index, num_classes=self.num_experts)
        ...
        if self.second_expert_policy == "random":
            top_2_max_probs = (router_probs * top_2_mask).sum(axis=1)
            sampled = (2 * top_2_max_probs) > ops.rand_like(top_2_max_probs.float())
            top_2_mask = top_2_mask * ops.transpose(sampled.tile((self.num_experts, 1)), 1, 0)
        ...
        if self.batch_prioritized_routing:
            ...
            locations1 = ops.cumsum(top_1_mask, dim=0) - 1
            locations2 = ops.cumsum(top_2_mask, dim=0) - 1
            # Update 2nd's location by accounting for locations of 1st
            locations2 += ops.sum(top_1_mask, dim=0, keepdim=True)
        ...
        top_1_mask = top_1_mask * ops.lt(locations1, self.expert_capacity)
        top_2_mask = top_2_mask * ops.lt(locations2, self.expert_capacity)
    ...
    def forward(
        ...
        Returns:
            top_1_mask (`mindspore.Tensor` of shape (batch_size, sequence_length)):
            ...
            router_probabilities (`mindspore.Tensor` of shape (batch_size, sequence_length, nump_experts)):
                Tensor of shape (batch_size, sequence_length, num_experts) corresponding to the probabilities for each
                token and expert. Used for routing tokens to experts.
            router_logits (`mindspore.Tensor` of shape (batch_size, sequence_length))):
...
class NllbMoeSparseMLP(nn.Module):
    ...
    def forward(
        self,
        hidden_states: mindspore.Tensor,
        padding_mask: Optional[mindspore.Tensor] = False,
    ):
        r"""
        ...
        Args:
            hidden_states (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_dim)`):
                The hidden states
            padding_mask (`mindspore.Tensor`, *optional*, defaults to `False`):
                Attention mask. Can be in the causal form or not.

        Returns:
            hidden_states (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_dim)`):
                Updated hidden states
            router_logits (`mindspore.Tensor` of shape `(batch_size, sequence_length, num_experts)`):
...
class NllbMoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            cross_attn_layer_head_mask (`mindspore.Tensor`):
...
class NllbMoeForConditionalGeneration(NllbMoePreTrainedModel):
    ...
    def _unpack_router_logits(self, router_outputs):
        ...
        total_router_logits = (
            ops.cat(total_router_logits, dim=1)
            if len(total_router_logits) > 0
            else None
        )
        total_expert_indexes = (
            ops.stack(total_expert_indexes, dim=1)
