...
class Attention(nn.Module):
    def __init__(self, nx, n_positions, config, scale=False):
        ...
        self.register_buffer(
            "bias",
            ops.tril(ops.ones(n_positions, n_positions)).view(1, 1, n_positions, n_positions),
            persistent=False,
        )
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    def __init__(self, config):
        ...
        self.register_buffer("position_ids", ops.arange(config.n_positions), persistent=False)
...
class OpenAIGPTDoubleHeadsModel(OpenAIGPTPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[mindspore.Tensor], OpenAIGPTDoubleHeadsModelOutput]:
        r"""
        ...
        >>> input_ids = mindspore.tensor([tokenizer.encode(s) for s in choices]).unsqueeze(0)  # Batch size 1, 2 choices
        >>> mc_token_ids = mindspore.tensor([input_ids.shape[-1] - 1, input_ids.shape[-1] - 1]).unsqueeze(0)  # Batch size 1
