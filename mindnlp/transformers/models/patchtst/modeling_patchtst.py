...
class PatchTSTBatchNorm(nn.Module):
    ...
    def forward(self, inputs: mindspore.Tensor):
        """
        Parameters:
            inputs (`mindspore.Tensor` of shape `(batch_size, sequence_length, d_model)`):
                input for Batch norm calculation
        Returns:
            `mindspore.Tensor` of shape `(batch_size, sequence_length, d_model)`
        """
        output = ops.transpose(inputs, 1, 2)  # output: (batch_size, d_model, sequence_length)
...
def random_masking(
    ...
    Args:
        inputs (`mindspore.Tensor` of shape `(batch_size, num_channels, sequence_length, num_features)`):
    ...
    Returns:
        `tuple(mindspore.Tensor)`: inputs_mask, masked input, same shape as input Tensor and mask tensor of shape [bs x c x
        n]
    """
    ...
    if channel_consistent_masking:
        noise = ops.rand(batch_size, 1, sequence_length)  # noise in [0, 1], bs x 1 x  L
        noise = noise.tile((1, num_channels, 1))  # bs x num_channels x time
    else:
        # noise in [0, 1], bs x num_channels x L
        noise = ops.rand(batch_size, num_channels, sequence_length)

    # mask: [bs x num_channels x num_patch]
    mask = ops.ones(batch_size, num_channels, sequence_length)
    mask[:, :, :len_keep] = 0

    # sort noise for each sample
    ids_shuffle = ops.argsort(noise, dim=-1)  # ascend: small is keep, large is remove
    ids_restore = ops.argsort(ids_shuffle, dim=-1)  # ids_restore: [bs x num_channels x L]

    mask = ops.gather(mask, dim=-1, index=ids_restore)
    mask = mask.unsqueeze(-1).tile((1, 1, 1, num_features))  # mask: [bs x num_channels x num_patches x patch_length]
...
def forecast_masking(
    ...
    Returns:
        `tuple(mindspore.Tensor)`: inputs_mask, masked input, same shape as inputs Tensor and Mask tensor of shape `(bs,
    ...
    batch_size, num_channels, sequence_length, num_features = inputs.shape
    mask = ops.zeros(batch_size, num_channels, sequence_length)
    ...
    perm = ops.randperm(mask.shape[0])
    mask = mask[perm]

    mask = mask.unsqueeze(-1).tile((1, 1, 1, num_features))  # mask: [bs x num_channels x num_patch x patch_len]
    if unmasked_channel_indices is not None:
        mask[:, unmasked_channel_indices, :, :] = 0
...
class PatchTSTPatchify(nn.Module):
    ...
    Returns:
        `mindspore.Tensor` of shape `(batch_size, num_channels, num_patches, patch_length)`
    """
    ...
    def forward(self, past_values: mindspore.Tensor):
        """
        Parameters:
            past_values (`mindspore.Tensor` of shape `(batch_size, sequence_length, num_channels)`, *required*):
        ...
        output = ops.unfold(output, dimension=-2, size=self.patch_length, step=self.patch_stride)
        # output: [bs x num_input_channels x num_patches x patch_length]
        output = ops.transpose(output, -2, -3)
        return output
...
class PatchTSTMasking(nn.Module):
    ...
    Returns:
        x_mask (`mindspore.Tensor` of shape `(batch_size, num_channels, num_patches, patch_length)`)
            Masked patched input
        mask (`mindspore.Tensor` of shape `(batch_size, num_channels, num_patches)`)
    ...
    def forward(self, patch_input: mindspore.Tensor):
        """
        Parameters:
            patch_input (`mindspore.Tensor` of shape `(batch_size, num_channels, num_patches, patch_length)`, *required*):
                Patch input

        Return:
            masked_input (`mindspore.Tensor` of shape `(batch_size, num_channels, num_patches, patch_length)`)
                Masked patched input
            mask (`mindspore.Tensor` of shape `(batch_size, num_channels, num_patches)`)
                Bool tensor indicating True on masked points
...
class PatchTSTEncoderLayer(nn.Module):
    ...
    def forward(self, hidden_state: mindspore.Tensor, output_attentions: Optional[bool] = None):
        """
        Parameters:
            hidden_state (`mindspore.Tensor` of shape `(batch_size, num_channels, sequence_length, d_model)`, *required*):
                Past values of the time series
            output_attentions (`bool`, *optional*):
                Whether or not to return the output attention of all layers
        Return:
            `mindspore.Tensor` of shape `(batch_size, num_channels, sequence_length, d_model)`
        ...
        if self.channel_attention:
            ...
            hidden_state = ops.transpose(hidden_state, 2, 1)
            ...
            # hidden_state: [bs x num_channels x sequence_length x d_model]
            hidden_state = ops.transpose(hidden_state, 1, 2)
...
class PatchTSTEmbedding(nn.Module):
    ...
    def forward(self, patch_input: mindspore.Tensor):
        ...
        return:
            `mindspore.Tensor` of shape `(batch_size, num_channels, num_patches, d_model)`
        """
        # Input encoding
        ...
        if self.share_embedding:
            ...
            embeddings = ops.stack(embeddings, dim=1)
        return embeddings
...
class PatchTSTPositionalEncoding(nn.Module):
    ...
    def __init__(self, config: PatchTSTConfig, num_patches: int):
        ...
        if config.use_cls_token:
            ...
            self.cls_token = nn.Parameter(ops.zeros(1, 1, 1, config.d_model))
            num_patches += 1
    ...
    def _init_pe(config: PatchTSTConfig, num_patches: int) -> nn.Parameter:
        ...
        if config.positional_encoding_type == "random":
            position_enc = nn.Parameter(ops.randn(num_patches, config.d_model), requires_grad=True)
        elif config.positional_encoding_type == "sincos":
            position_enc = ops.zeros(num_patches, config.d_model)
            position = ops.arange(0, num_patches).unsqueeze(1)
            div_term = ops.exp(ops.arange(0, config.d_model, 2) * -(math.log(10000.0) / config.d_model))
            position_enc[:, 0::2] = ops.sin(position * div_term)
            position_enc[:, 1::2] = ops.cos(position * div_term)
    ...
    def forward(self, patch_input: mindspore.Tensor):
        if self.use_cls_token:
            ...
            hidden_state = ops.cat((cls_tokens, patch_input), dim=2)
        else:
...
class PatchTSTModelOutput(ModelOutput):
    ...
    Parameters:
        last_hidden_state (`mindspore.Tensor` of shape `(batch_size, num_channels, num_patches, patch_length)`):
        ...
        mask: (`mindspore.Tensor` of shape `(batch_size, num_channels, num_patches)`, *optional*)
            Bool masked tensor indicating which patches are masked
        loc: (`mindspore.Tensor` of shape `(batch_size, 1, num_channels)`, *optional*)
            Mean of the input data (batch_size, sequence_length, num_channels) over the sequence_length
        scale: (`mindspore.Tensor` of shape `(batch_size, 1, num_channels)`, *optional*)
            Std of the input data (batch_size, sequence_length, num_channels) over the sequence_length
        patch_input (`mindspore.Tensor` of shape `(batch_size, num_channels, num_patches, patch_length)`):
    ...
    mask: mindspore.Tensor = None
    loc: mindspore.Tensor = None
    scale: mindspore.Tensor = None
    patch_input: mindspore.Tensor = None
...
class PatchTSTForPretrainingOutput(ModelOutput):
    ...
    Parameters:
        loss (*optional*, returned when `labels` is provided, `mindspore.Tensor` of shape `(1,)`):
            MSE loss.
        prediction_outputs (`mindspore.Tensor` of shape `(batch_size, sequence_length, config.vocab_size)`):
    ...
    prediction_output: mindspore.Tensor = None
    hidden_states: Optional[Tuple[mindspore.Tensor]] = None
    attentions: Optional[Tuple[mindspore.Tensor]] = None
...
class PatchTSTForRegressionOutput(ModelOutput):
    ...
    Parameters:
        loss (*optional*, returned when `labels` is provided, `mindspore.Tensor` of shape `(1,)`):
            MSE loss.
        regression_outputs (`mindspore.Tensor` of shape `(batch_size, num_targets)`):
    ...
    loss: Optional[mindspore.Tensor] = None
    regression_outputs: mindspore.Tensor = None
    hidden_states: Optional[Tuple[mindspore.Tensor]] = None
    attentions: Optional[Tuple[mindspore.Tensor]] = None
...
class PatchTSTForPredictionOutput(ModelOutput):
    ...
    Parameters:
        loss (*optional*, returned when `labels` is provided, `mindspore.Tensor` of shape `(1,)`):
            MSE loss.
        prediction_outputs (`mindspore.Tensor` of shape `(batch_size, prediction_length, -1)`):
            Prediction outputs of the time series modeling heads.
        hidden_states (`tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `mindspore.Tensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, sequence_length, hidden_size)`.
    ...
    loss: Optional[mindspore.Tensor] = None
    prediction_outputs: mindspore.Tensor = None
    hidden_states: Optional[Tuple[mindspore.Tensor]] = None
    attentions: Optional[Tuple[mindspore.Tensor]] = None
    loc: mindspore.Tensor = None
    scale: mindspore.Tensor = None
...
class PatchTSTForClassificationOutput(ModelOutput):
    ...
    Parameters:
        loss (*optional*, returned when `labels` is provided, `mindspore.Tensor` of shape `(1,)`):
            ...
            (classification) loss.
        prediction_logits (`mindspore.Tensor` of shape `(batch_size, num_targets)`):
...
def nll(input: Distribution, target: mindspore.Tensor) -> mindspore.Tensor:
    """
...
class PatchTSTStdScaler(nn.Module):
    ...
    def forward(
        ...
        denominator = ops.clamp(denominator, 1.0)
...
class PatchTSTModel(PatchTSTPreTrainedModel):
    ...
    def forward(
        ...
        r"""
        Parameters:
            past_values (`mindspore.Tensor` of shape `(bs, sequence_length, num_input_channels)`, *required*):
                Input sequence to the model
            past_observed_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length, num_input_channels)`, *optional*):
                Boolean mask to indicate which `past_values` were observed and which were missing. Mask values selected
                in `[0, 1]`:

                - 1 for values that are **observed**,
                - 0 for values that are **missing** (i.e. NaNs that were replaced by zeros).
            future_values (`mindspore.Tensor` of shape `(batch_size, prediction_length, num_input_channels)`, *optional*):
        ...
        Returns:
            `PatchTSTModelOutput` or tuple of `mindspore.Tensor` (if `return_dict`=False or `config.return_dict`=False)
...
class PatchTSTMaskPretrainHead(nn.Module):
    ...
    def forward(self, embedding: mindspore.Tensor) -> mindspore.Tensor:
        """
        Parameters:
            embedding (`mindspore.Tensor` of shape `(bs, num_channels, num_patches, d_model)` or
                    `(bs, num_channels, num_patches+1, d_model)` if `cls_token` is set to True, *required*):
                Embedding from the model
        Returns:
            `mindspore.Tensor` of shape `(bs, num_channels, num_patches, d_model)` or
...
class PatchTSTForPretraining(PatchTSTPreTrainedModel):
    ...
    def forward(
        ...
        Returns:
            `PatchTSTForPretrainingOutput` or tuple of `mindspore.Tensor` (if `return_dict`=False or
        ...
        masked_loss = (ops.mean(loss_val, dim=-1) * model_output.mask).sum() / (model_output.mask.sum() + 1e-10)
...
class PatchTSTClassificationHead(nn.Module):
    ...
    def forward(self, embedding: mindspore.Tensor):
        """
        Parameters:
            embedding (`mindspore.Tensor` of shape `(bs, num_channels, num_patches, d_model)` or
                     `(bs, num_channels, num_patches+1, d_model)` if `cls_token` is set to True, *required*):
                Embedding from the model
        Returns:
            `mindspore.Tensor` of shape `(bs, num_targets)`
        ...
        if self.use_cls_token:
            ...
            pooled_embedding = ops.mean(embedding, dim=2)
        elif self.pooling_type == "max":
            # pooled_embedding: [bs x num_channels x d_model]
            pooled_embedding = ops.max(embedding, dim=2)[0]
        else:
...
class PatchTSTForClassification(PatchTSTPreTrainedModel):
    ...
    def forward(
        self,
        past_values: mindspore.Tensor,
        target_values: mindspore.Tensor = None,
        ...
        Parameters:
            past_values (`mindspore.Tensor` of shape `(bs, sequence_length, num_input_channels)`, *required*):
                Input sequence to the model
            target_values (`mindspore.Tensor`, *optional*):
        ...
        Returns:
            `PatchTSTForClassificationOutput` or tuple of `mindspore.Tensor` (if `return_dict`=False or
            `config.return_dict`=False)
        ...
        >>> past_values = ops.randn(20, 512, 2)
    ...
    def forward(self, embedding: mindspore.Tensor):
        ...
        Parameters:
            embedding (`mindspore.Tensor` of shape `(bs, num_channels, num_patches, d_model)` or
                ...
                Embedding from the model
        Returns:
            `mindspore.Tensor` of shape `(bs, forecast_len, num_channels)`
        ...
        if isinstance(output, tuple):
            ...
            output = tuple(ops.transpose(z, 2, 1) for z in output)
        else:
            output = ops.transpose(output, 2, 1)  # [bs x forecast_len x num_channels]
        return output
...
class PatchTSTForPrediction(PatchTSTPreTrainedModel):
    ...
    def forward(
        ...
        Parameters:
            ...
            past_observed_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length, num_input_channels)`, *optional*):
                ...
                - 1 for values that are **observed**,
                - 0 for values that are **missing** (i.e. NaNs that were replaced by zeros).
            future_values (`mindspore.Tensor` of shape `(bs, forecast_len, num_input_channels)`, *optional*):
                Future target values associated with the `past_values`
            output_hidden_states (`bool`, *optional*):
                Whether or not to return the hidden states of all layers
            output_attentions (`bool`, *optional*):
        ...
        Returns:
            `PatchTSTForPredictionOutput` or tuple of `mindspore.Tensor` (if `return_dict`=False or
            `config.return_dict`=False)
    ...
    def generate(
        ...
        Parameters:
            past_values (`mindspore.Tensor` of shape `(batch_size, sequence_length, num_input_channels)`):
        ...
        if self.distribution_output:
            ...
            samples = ops.stack(samples, dim=1)
        else:
    ...
    def forward(self, embedding: mindspore.Tensor):
        ...
        Parameters:
            embedding (`mindspore.Tensor` of shape `(bs, num_channels, num_patches, d_model)` or
                ...
                Embedding from the model
        Returns:
            `mindspore.Tensor` of shape `(bs, output_dim)`
        ...
        if (self.distribution_output is None) & (self.y_range is not None):  # linear head
            output = ops.sigmoid(output) * (self.y_range[1] - self.y_range[0]) + self.y_range[0]
        return output
...
class PatchTSTForRegression(PatchTSTPreTrainedModel):
    ...
    def forward(
        ...
        Parameters:
            past_values (`mindspore.Tensor` of shape `(bs, sequence_length, num_input_channels)`, *required*):
                Input sequence to the model
            target_values (`mindspore.Tensor` of shape `(bs, num_input_channels)`):
        ...
        Returns:
            `PatchTSTForRegressionOutput` or tuple of `mindspore.Tensor` (if `return_dict`=False or
            `config.return_dict`=False)
        ...
        >>> # during inference, one only provides past values, the model outputs future values
        >>> past_values = ops.randn(20, 512, 6)
        >>> outputs = model(past_values=past_values)
    ...
    def generate(
        ...
        samples = ops.stack(samples, dim=1).view(-1, num_parallel_samples, self.config.num_targets)
        return SamplePatchTSTOutput(sequences=samples)
