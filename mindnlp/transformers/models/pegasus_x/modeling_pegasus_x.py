...
class PegasusXSinusoidalPositionalEmbedding(nn.Module):
    ...
    def forward(self, input_embeds: mindspore.Tensor, past_key_values_length: int = 0) -> mindspore.Tensor:
        ...
        pe[:, :half_d_feature] = ops.sin(positions * div_term)
        pe[:, half_d_feature:] = ops.cos(positions * div_term)
        return pe[None].broadcast_to((batch_size, -1, -1))
...
class PegasusXGlobalLocalAttention(nn.Module):
    ...
    def forward(
        self,
        token_hidden_states: mindspore.Tensor,
        global_hidden_states: mindspore.Tensor,
        attention_mask: Optional[mindspore.Tensor] = None,
        output_attentions: bool = False,
    ) -> Tuple[mindspore.Tensor, mindspore.Tensor, Optional[mindspore.Tensor]]:
        """Input shape: Batch x Time x Channel"""
        dim = DimensionInfo(
        ...
        global_attn_output = (
            ops.transpose(global_attn_output, 1, 2).view(dim.batch_size, dim.global_len, dim.hidden_dim)
        )
        # [batch_size, global_len, hidden_dim]
        global_attn_output = self.out_proj(global_attn_output)
        # [batch_size, num_heads, block_size, num_heads, dim_per_head]
        local_attn_output = local_attn_output.permute(0, 2, 3, 1, 4)
    ...
    def compute_global_attention_representations(
        ...
        Args:
            global_q (`mindspore.Tensor`) of shape [batch_size, num_heads, global_len, dim_per_head]:
                query vectors from global tokens
            global_k (`mindspore.Tensor`) of shape [batch_size, num_heads, global_len, dim_per_head]:
                key vectors from global tokens
            global_v (`mindspore.Tensor`) of shape [batch_size, num_heads, global_len, dim_per_head]:
                value vectors from global tokens
            local_k (`mindspore.Tensor`) of shape [batch_size, num_heads, padded_seq_len, dim_per_head]:
                key vectors from local tokens
            local_v (`mindspore.Tensor`) of shape [batch_size, num_heads, padded_seq_len, dim_per_head]:
                value vectors from local tokens
            mask (`mindspore.Tensor`) of shape [batch_size, padded_seq_len]: attention mask
        ...
        global_and_local_k = ops.cat([global_k, local_k], dim=2)
        # [batch_size, num_heads, global_len+padded_seq_len, dim_per_head]
        global_and_local_v = ops.cat([global_v, local_v], dim=2)

        # [batch_size, global_len+padded_seq_len]
        extended_mask = F.pad(mask, pad=(dim.global_len, 0), value=0)

        # [batch_size, num_heads, global_len, global_len+padded_seq_len]
        attn_weights = ops.einsum("BHGF,BHXF->BHGX", global_q, global_and_local_k)
        attn_weights = attn_weights + extended_mask[:, None, None, :]
        attn_probs = F.softmax(attn_weights, dim=-1)
        attn_probs = F.dropout(attn_probs, p=self.dropout, training=self.training)

        # [batch_size, num_heads, global_len, F]
        attn_output = ops.einsum("BHGX,BHXF->BHGF", attn_probs, global_and_local_v)
    ...
    def compute_local_attention_representations(
        ...
        Args:
            ...
            global_v (`mindspore.Tensor`) of shape [batch_size, num_heads, global_len, dim_per_head]:
                value vectors from global tokens
            local_q (`mindspore.Tensor`) of shape [batch_size, num_heads, padded_seq_len, dim_per_head]:
        ...
        attn_weights = ops.cat([blocked_local2global, blocked_local2local], dim=-1)
        attn_weights = attn_weights + extended_mask[:, None, :, None, :]
        attn_probs = F.softmax(attn_weights, dim=-1)
        attn_probs = F.dropout(attn_probs, p=self.dropout, training=self.training)
...
class PegasusXEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            global_hidden_states (`mindspore.Tensor`): global token hidden states
        ...
        global_hidden_states = F.dropout(global_hidden_states, p=self.dropout, training=self.training)
        ...
        global_hidden_states = F.dropout(
...
class PegasusXEncoder(PegasusXPreTrainedModel):
    ...
    def forward(
        ...
        if seq_len % self.config.block_size != 0:
            ...
            hidden_states = F.pad(hidden_states, pad=(0, 0, 0, pad_len), value=0)
            attention_mask = F.pad(attention_mask, pad=(0, pad_len), value=mask_min_value)
