...
class PerceiverImageProcessor(BaseImageProcessor):
    ...
    Args:
        do_center_crop (`bool`, `optional`, defaults to `True`):
            Whether to center crop the image. If the input size if smaller than `crop_size` along any edge, the
    ...
    # Copied from transformers.models.vit.image_processing_vit.ViTImageProcessor.resize with
    # PILImageResampling.BILINEAR->PILImageResampling.BICUBIC
    def resize(
            self,
            image: np.ndarray,
            size: Dict[str, int],
