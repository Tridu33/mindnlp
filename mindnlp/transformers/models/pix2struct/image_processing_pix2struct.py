...
def torch_extract_patches(image_tensor, patch_height, patch_width):
    ...
    """
    requires_backends(torch_extract_patches, ["mindspore"])

    image_tensor = image_tensor.unsqueeze(0)
    patches = F.unfold(image_tensor, (patch_height, patch_width), stride=(patch_height, patch_width))
    patches = patches.reshape(image_tensor.shape[0], image_tensor.shape[1], patch_height, patch_width, -1)
    patches = patches.permute(0, 4, 2, 3, 1).reshape(
        image_tensor.shape[2] // patch_height,
        image_tensor.shape[3] // patch_width,
        image_tensor.shape[1] * patch_height * patch_width,
    )
...
class Pix2StructImageProcessor(BaseImageProcessor):
    ...
    def extract_flattened_patches(
        ...
        **kwargs,
        ...
        """
        requires_backends(self.extract_flattened_patches, "mindspore")

        # convert to torch
        image = to_channel_dimension_format(image, ChannelDimension.FIRST, input_data_format)
        image = mindspore.Tensor.from_numpy(image)
        ...
        # [rows * columns, 1]
        row_ids = ops.arange(rows).reshape([rows, 1]).tile((1, columns)).reshape([rows * columns, 1])
        col_ids = ops.arange(columns).reshape([1, columns]).tile((rows, 1)).reshape([rows * columns, 1])
        ...
        # [rows * columns, 1]
        row_ids = row_ids.to(mindspore.float32)
        col_ids = col_ids.to(mindspore.float32)

        # [rows * columns, 2 + patch_height * patch_width * image_channels]
        result = ops.cat([row_ids, col_ids, patches], -1)
