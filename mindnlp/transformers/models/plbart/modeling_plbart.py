...
class PLBartEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if hidden_states.dtype == ms.float16 and (
            ...
            clamp_value = np.finfo(ms.dtype_to_nptype(hidden_states.dtype)).max - 1000
        ...
        if isinstance(module, nn.Linear):
            ...
            if module.padding_idx is not None:
                module.weight[module.padding_idx] = initializer(
                    "zeros", module.weight[module.padding_idx].shape, module.weight.dtype)
...
PLBART_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_attention_mask (:
            obj:*ms.Tensor* of shape `(batch_size, target_sequence_length)`, *optional*): Default behavior:
        ...
        past_key_values (:
            obj:*tuple(tuple(ms.Tensor))*, *optional*, returned when `use_cache=True` is passed or when
            `config.use_cache=True`): Tuple of `tuple(ms.Tensor)` of length `config.n_layers`, with each tuple
        ...
        inputs_embeds (:
            obj:*ms.Tensor* of shape `(batch_size, sequence_length, hidden_size)`, *optional*): Optionally,
        ...
        decoder_inputs_embeds (:
            obj:*ms.Tensor* of shape `(batch_size, target_sequence_length, hidden_size)`, *optional*):
...
__all__ = ["PLBartForCausalLM", "PLBartForConditionalGeneration", "PLBartForSequenceClassification", "PLBartModel", "PLBartPreTrainedModel"]
