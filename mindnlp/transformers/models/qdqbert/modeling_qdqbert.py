...
class QDQBertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in QDQBertModel contrust() function)
...
class QDQBertModel(QDQBertPreTrainedModel):
    ...
    `add_cross_attention` set to `True`; an `encoder_hidden_states` is then expected as an input to the contrust pass.
    """
