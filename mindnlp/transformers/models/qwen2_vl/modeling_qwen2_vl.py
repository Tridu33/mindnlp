...
class Qwen2VLCausalLMOutputWithPast(ModelOutput):
    ...
    Args:
        ...
        attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            heads.
        rope_deltas (`mindspore.Tensor` of shape `(batch_size, )`, *optional*):
    ...
    rope_deltas: Optional[mindspore.Tensor] = None
...
class VisionRotaryEmbedding(nn.Module):
    def __init__(self, dim: int, theta: float = 10000.0) -> None:
        super().__init__()
        inv_freq = 1.0 / (theta ** (ops.arange(0, dim, 2, dtype=mindspore.float32) / dim))
        self.register_buffer("inv_freq", inv_freq, persistent=False)

    def forward(self, seqlen: int) -> mindspore.Tensor:
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def rot_pos_emb(self, grid_thw):
        ...
        pos_ids = ops.cat(pos_ids, dim=0)
        max_grid_size = grid_thw[:, 1:].max()
        rotary_pos_emb_full = self.rotary_pos_emb(max_grid_size.item())
        rotary_pos_emb = ops.flatten(rotary_pos_emb_full[pos_ids], 1)
        return rotary_pos_emb

    def forward(self, hidden_states: mindspore.Tensor, grid_thw: mindspore.Tensor) -> mindspore.Tensor:
        hidden_states = self.patch_embed(hidden_states)
        rotary_pos_emb = self.rot_pos_emb(grid_thw)

        cu_seqlens = ops.cumsum(
            ops.repeat_interleave(grid_thw[:, 1] * grid_thw[:, 2], grid_thw[:, 0].tolist()),
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel):
    ...
    def get_rope_index(
        ...
        video_grid_thw: Optional[mindspore.Tensor] = None,
        attention_mask: Optional[mindspore.Tensor] = None,
        ...
        Args:
            input_ids (`mindspore.Tensor` of shape `(batch_size, sequence_length)`):
                ...
                it.
            image_grid_thw (`mindspore.Tensor` of shape `(num_images, 3)`, *optional*):
                The temporal, height and width of feature shape of each image in LLM.
            video_grid_thw (`mindspore.Tensor` of shape `(num_videos, 3)`, *optional*):
        ...
        Returns:
            position_ids (`mindspore.Tensor` of shape `(3, batch_size, sequence_length)`)
        ...
        if image_grid_thw is not None or video_grid_thw is not None:
            ...
            if attention_mask is not None:
                ...
                max_position_ids = ops.max(ops.max(position_ids.long(), 0, keepdim=False)[0], -1, keepdim=True)[0]
    ...
    def forward(
        ...
        pixel_values_videos: Optional[mindspore.Tensor] = None,
        image_grid_thw: Optional[mindspore.Tensor] = None,
        video_grid_thw: Optional[mindspore.Tensor] = None,
        rope_deltas: Optional[mindspore.Tensor] = None,
