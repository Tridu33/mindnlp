...
class RealmEmbedderOutput(ModelOutput):
    ...
    Args:
        projected_score (`mindspore.Tensor` of shape `(batch_size, config.retriever_proj_size)`):
    ...
    projected_score: mindspore.Tensor = None
...
class RealmScorerOutput(ModelOutput):
    ...
    Args:
        relevance_score (`mindspore.Tensor` of shape `(batch_size, config.num_candidates)`):
            The relevance score of document candidates (before softmax).
        query_score (`mindspore.Tensor` of shape `(batch_size, config.retriever_proj_size)`):
            Query score derived from the query embedder.
        candidate_score (`mindspore.Tensor` of shape `(batch_size, config.num_candidates, config.retriever_proj_size)`):
            Candidate score derived from the embedder.
    """

    relevance_score: mindspore.Tensor = None
    query_score: mindspore.Tensor = None
    candidate_score: mindspore.Tensor = None
...
class RealmReaderOutput(ModelOutput):
    ...
    Args:
        loss (`mindspore.Tensor` of shape `(1,)`, *optional*, returned when `start_positions`, `end_positions`, `has_answers` are provided):
            Total loss.
        retriever_loss (`mindspore.Tensor` of shape `(1,)`, *optional*, returned when `start_positions`, `end_positions`, `has_answers` are provided):
            Retriever loss.
        reader_loss (`mindspore.Tensor` of shape `(1,)`, *optional*, returned when `start_positions`, `end_positions`, `has_answers` are provided):
            Reader loss.
        retriever_correct (`mindspore.Tensor` of shape `(config.searcher_beam_size,)`, *optional*):
            Whether or not an evidence block contains answer.
        reader_correct (`mindspore.Tensor` of shape `(config.reader_beam_size, num_candidates)`, *optional*):
            Whether or not a span candidate contains answer.
        block_idx (`mindspore.Tensor` of shape `()`):
            The index of the retrieved evidence block in which the predicted answer is most likely.
        candidate (`mindspore.Tensor` of shape `()`):
    ...
    retriever_loss: mindspore.Tensor = None
    reader_loss: mindspore.Tensor = None
    retriever_correct: mindspore.Tensor = None
    reader_correct: mindspore.Tensor = None
    block_idx: mindspore.Tensor = None
    candidate: mindspore.Tensor = None
    start_pos: mindspore.int32 = None
    end_pos: mindspore.int32 = None
    hidden_states: Optional[Tuple[mindspore.Tensor]] = None
    attentions: Optional[Tuple[mindspore.Tensor]] = None
...
class RealmForOpenQAOutput(ModelOutput):
    ...
    Args:
        reader_output (`dict`):
            Reader output.
        predicted_answer_ids (`mindspore.Tensor` of shape `(answer_sequence_length)`):
    ...
    def forward(self, hidden_states, block_mask):
        def span_candidates(masks):
            ...
            def _spans_given_width(width):
                current_starts = ops.arange(max_sequence_len - width + 1)
                current_ends = ops.arange(width - 1, max_sequence_len)
                return current_starts, current_ends

            starts, ends = zip(*(_spans_given_width(w + 1) for w in range(self.config.max_span_width)))

            # [num_spans]
            starts = ops.cat(starts, 0)
            ends = ops.cat(ends, 0)
        ...
        start_projection, end_projection = hidden_states.chunk(2, axis=-1)
...
class RealmScorer(RealmPreTrainedModel):
    ...
    def forward(
        ...
        token_type_ids: Optional[mindspore.Tensor] = None,
        position_ids: Optional[mindspore.Tensor] = None,
        candidate_input_ids: Optional[mindspore.Tensor] = None,
        candidate_attention_mask: Optional[mindspore.Tensor] = None,
        candidate_token_type_ids: Optional[mindspore.Tensor] = None,
        candidate_inputs_embeds: Optional[mindspore.Tensor] = None,
        head_mask: Optional[mindspore.Tensor] = None,
        inputs_embeds: Optional[mindspore.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        ...
        r"""
        candidate_input_ids (`mindspore.Tensor` of shape `(batch_size, num_candidates, sequence_length)`):
        ...
        candidate_attention_mask (`mindspore.Tensor` of shape `(batch_size, num_candidates, sequence_length)`, *optional*):
        ...
        candidate_token_type_ids (`mindspore.Tensor` of shape `(batch_size, num_candidates, sequence_length)`, *optional*):
        ...
        candidate_inputs_embeds (`mindspore.Tensor` of shape `(batch_size * num_candidates, sequence_length, hidden_size)`, *optional*):
        ...
        relevance_score = ops.einsum("bd,bnd->bn", query_score, candidate_score)
...
class RealmKnowledgeAugEncoder(RealmPreTrainedModel):
    ...
    def forward(
        ...
        head_mask: Optional[mindspore.Tensor] = None,
        inputs_embeds: Optional[mindspore.Tensor] = None,
        relevance_score: Optional[mindspore.Tensor] = None,
        labels: Optional[mindspore.Tensor] = None,
        mlm_mask: Optional[mindspore.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, MaskedLMOutput]:
        r"""
        relevance_score (`mindspore.Tensor` of shape `(batch_size, num_candidates)`, *optional*):
        ...
        mlm_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
        ...
        if labels is not None:
            ...
            if mlm_mask is None:
                mlm_mask = ops.ones_like(labels, dtype=mindspore.float32)
            else:
                mlm_mask = mlm_mask.type(mindspore.float32)

            # Compute marginal log-likelihood
            # loss_fct = cross_entropy(reduction="none")  # -100 index = padding token
...
class RealmReader(RealmPreTrainedModel):
    ...
    def forward(
        ...
        inputs_embeds: Optional[mindspore.Tensor] = None,
        relevance_score: Optional[mindspore.Tensor] = None,
        block_mask: Optional[mindspore.Tensor] = None,
        start_positions: Optional[mindspore.Tensor] = None,
        end_positions: Optional[mindspore.Tensor] = None,
        has_answers: Optional[mindspore.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, RealmReaderOutput]:
        r"""
        relevance_score (`mindspore.Tensor` of shape `(searcher_beam_size,)`, *optional*):
            Relevance score, which must be specified if you want to compute the logits and marginal log loss.
        block_mask (`mindspore.Tensor` of shape `(searcher_beam_size, sequence_length)`, *optional*):
            The mask of the evidence block, which must be specified if you want to compute the logits and marginal log
            loss.
        start_positions (`mindspore.Tensor` of shape `(searcher_beam_size,)`, *optional*):
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        end_positions (`mindspore.Tensor` of shape `(searcher_beam_size,)`, *optional*):
            Labels for position (index) of the end of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        has_answers (`mindspore.Tensor` of shape `(searcher_beam_size,)`, *optional*):
        ...
        if token_type_ids.shape[1] < self.config.max_span_width:
        ...
        if start_positions is not None and end_positions is not None and has_answers is not None:
            ...
            def compute_correct_candidates(candidate_starts, candidate_ends, gold_starts, gold_ends):
                ...
                is_gold_start = ops.eq(
                    ops.unsqueeze(ops.unsqueeze(candidate_starts, 0), 0), ops.unsqueeze(gold_starts, -1)
                )
                is_gold_end = ops.eq(
                    ops.unsqueeze(ops.unsqueeze(candidate_ends, 0), 0), ops.unsqueeze(gold_ends, -1)
                )

                # [reader_beam_size, num_candidates]
                return ops.any(ops.logical_and(is_gold_start, is_gold_end), 1)
            ...
            ignored_index = sequence_output.shape[1]
            ...
            any_retriever_correct = ops.any(retriever_correct)
            ...
            any_reader_correct = ops.any(reader_correct)
            ...
            retriever_loss *= any_retriever_correct.type(mindspore.float32)
            reader_loss *= any_reader_correct.type(mindspore.float32)
...
class RealmForOpenQA(RealmPreTrainedModel):
    ...
    def forward(
        ...
        attention_mask: Optional[mindspore.Tensor] = None,
        token_type_ids: Optional[mindspore.Tensor] = None,
        answer_ids: Optional[mindspore.Tensor] = None,
        return_dict: Optional[bool] = None,
        ...
        retrieved_logits = ops.einsum(
            "D,BD->B", question_projection.squeeze(), retrieved_block_emb
        )
