...
class SegGptEmbeddings(nn.Module):
    ...
    def __init__(self, config: SegGptConfig) -> None:
        ...
        self.mask_token = nn.Parameter(ops.zeros(1, 1, 1, config.hidden_size))
        self.segment_token_input = nn.Parameter(ops.zeros(1, 1, 1, config.hidden_size))
        self.segment_token_prompt = nn.Parameter(ops.zeros(1, 1, 1, config.hidden_size))
        # token for seg types
        self.type_token_semantic = nn.Parameter(ops.zeros(1, 1, 1, config.hidden_size))
        self.type_token_instance = nn.Parameter(ops.zeros(1, 1, 1, config.hidden_size))

        self.patch_embeddings = SegGptPatchEmbeddings(config)

        num_positions = (config.pretrain_image_size // config.patch_size) ** 2 + 1
        self.position_embeddings = nn.Parameter(ops.randn(1, num_positions, config.hidden_size))
...
class SegGptLayer(nn.Module):
    ...
    def forward(
        ...
        if feature_ensemble and attention_output.shape[0] // 2 >= ensemble_cond:
            prompt, inputs = ops.split(attention_output, attention_output.shape[1] // 2, dim=1)
            if ensemble_cond == 2:
                num_prompts = attention_output.shape[0] // 2
                inputs = inputs.reshape(2, num_prompts, -1)
                inputs = ops.mean(inputs, dim=1, keepdim=True).expand_as(inputs)
                inputs = inputs.reshape(*prompt.shape)
            else:
                inputs = ops.mean(inputs, dim=0, keepdim=True).expand_as(inputs)
...
class SegGptModel(SegGptPreTrainedModel):
    ...
    def forward(
        ...
        if bool_masked_pos is None:
            ...
            bool_masked_pos = ops.zeros(num_patches, dtype=mindspore.bool_)
...
def patchify(tensor: mindspore.Tensor, patch_size: int) -> mindspore.Tensor:
    ...
    tensor = tensor.reshape((batch_size, num_channels, patch_height, patch_size, patch_width, patch_size))
    tensor = tensor.permute(0, 2, 4, 3, 5, 1)
    tensor = tensor.reshape((batch_size, patch_height * patch_width, patch_size**2 * 3))
...
def unpatchify(tensor: mindspore.Tensor, patch_height: int, patch_width: int) -> mindspore.Tensor:
    ...
    tensor = tensor.reshape((batch_size, patch_height, patch_width, patch_size, patch_size, 3))
    tensor = tensor.permute(0, 5, 1, 3, 2, 4)
    tensor = tensor.reshape((batch_size, 3, patch_height * patch_size, patch_width * patch_size))
...
class SegGptLoss(nn.Module):
    ...
    def forward(
        ...
        mask = bool_masked_pos[:, :, None].tile((1, 1, self.patch_size**2 * 3))
...
class SegGptForImageSegmentation(SegGptPreTrainedModel):
    ...
    def forward(
        ...
        intermediate_hidden_states = ops.cat(intermediate_hidden_states, dim=-1)
