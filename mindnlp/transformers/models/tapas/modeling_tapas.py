    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        if token_type_ids is None:
            token_type_ids = ops.zeros(
                (input_shape + self.number_of_token_type_embeddings), dtype=mindspore.int64
            )
...
class TapasModel(TapasPreTrainedModel):
    ...
    def forward(
        ...
        if token_type_ids is None:
            token_type_ids = ops.zeros(
                (*input_shape, len(self.config.type_vocab_sizes)), dtype=mindspore.int64
            )
...
class TapasForQuestionAnswering(TapasPreTrainedModel):
    ...
    def forward(
        ...
        if table_mask is None:
            table_mask = ops.where(row_ids > 0, ops.ones_like(row_ids), ops.zeros_like(row_ids))
        # mindspore.Tensor[batch_size, seq_length]
        input_mask_float = attention_mask.float()
        table_mask_float = table_mask.float()
        ...
        if labels is not None:
            ...
            if not self.config.select_one_column:
                weight = ops.where(
                    labels == 0,
                    ops.ones_like(labels, dtype=mindspore.float32),
                    self.config.positive_label_weight * ops.ones_like(labels, dtype=mindspore.float32),
                )
                selection_loss_per_token = -dist_per_token.log_prob(labels) * weight
                selection_loss_per_example = ops.sum(selection_loss_per_token * input_mask_float, dim=1) / (
                    ops.sum(input_mask_float, dim=1) + EPSILON_ZERO_DIVISION
                )
            else:
            ...
            if self.config.disable_per_token_loss:
                ...
                total_loss += ops.mean(selection_loss_per_example)
            else:
                # For the not supervised case, do not assign loss for cell selection
                total_loss += ops.mean(selection_loss_per_example * (1.0 - aggregate_mask))
            ...
            if self.config.num_aggregation_labels > 0:
                if is_supervised:
                    ...
                    aggregation_labels = ops.zeros(labels.shape[0], dtype=mindspore.int64)
                    per_example_additional_loss = _calculate_aggregation_loss(
                        logits_aggregation,
                        aggregate_mask,
                        aggregation_labels,
...
class ProductIndexMap(IndexMap):
    ...
    def project_outer(self, index):
        ...
        indices = ops.div(index.indices, self.inner_index.num_segments, rounding_mode="floor").type(mindspore.int64)
...
def _single_column_cell_selection_loss(token_logits, column_logits, labels, cell_index, col_index, cell_mask):
    ...
    selected_column_mask = ops.where(
        ops.eq(column_id_for_cells, 0).view(selected_column_mask.shape),
        ops.zeros_like(selected_column_mask),
...
def compute_token_logits(sequence_output, temperature, output_weights, output_bias):
    ...
    """
    logits = (ops.einsum("bsj,j->bs", sequence_output, output_weights) + output_bias) / temperature
...
def _calculate_aggregation_loss_known(
    ...
    if use_answer_as_supervision:
        ...
        target_aggregation = ops.zeros_like(aggregate_mask, dtype=mindspore.int64)
    else:
...
def _calculate_aggregation_loss_unknown(logits_aggregation, aggregate_mask):
    ...
    # correct answer.
    return -ops.log(aggregation_ops_total_mass) * aggregate_mask
...
def _calculate_expected_result(
    ...
    count_result = ops.sum(scaled_probability_per_cell, dim=1)
    numeric_values_masked = ops.where(
        ops.isnan(numeric_values), ops.zeros_like(numeric_values), numeric_values
    )  # Mask non-numeric table values to zero.
    sum_result = ops.sum(scaled_probability_per_cell * numeric_values_masked, dim=1)
    ...
    if avg_approximation == AverageApproximationFunction.RATIO:
        ...
        ex = ops.sum(scaled_probability_per_cell, dim=1, keepdim=True) - scaled_probability_per_cell + 1
        average_result = ops.sum(numeric_values_masked * scaled_probability_per_cell / ex, dim=1)
    elif avg_approximation == AverageApproximationFunction.SECOND_ORDER:
        # The sum of all probabilities except that correspond to other cells
        ex = ops.sum(scaled_probability_per_cell, dim=1, keepdim=True) - scaled_probability_per_cell + 1
        pointwise_var = scaled_probability_per_cell * (1 - scaled_probability_per_cell)
        var = ops.sum(pointwise_var, dim=1, keepdim=True) - pointwise_var

        multiplier = (var / ops.square(ex) + 1) / ex
        average_result = ops.sum(numeric_values_masked * scaled_probability_per_cell * multiplier, dim=1)
    else:
    ...
    all_results = ops.cat(
        [
            ops.unsqueeze(sum_result, dim=1),
...
def huber_loss(input, target, delta: float = 1.0):
    errors = ops.abs(input - target)  # shape (batch_size,)
    return ops.where(errors < delta, 0.5 * errors**2, errors * delta - (0.5 * delta**2))
...
def _calculate_regression_loss(
    ...
    if config.answer_loss_cutoff is None:
        large_answer_loss_mask = ops.ones_like(per_example_answer_loss, dtype=mindspore.float32)

    else:
        large_answer_loss_mask = ops.where(
            per_example_answer_loss > config.answer_loss_cutoff,
            ops.zeros_like(per_example_answer_loss, dtype=mindspore.float32),
            ops.ones_like(per_example_answer_loss, dtype=mindspore.float32),
        )
