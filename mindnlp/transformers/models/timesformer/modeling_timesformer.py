...
class TimesformerSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, output_attentions: bool = False):
        ...
        attention_probs = (query @ ops.transpose(key, -2, -1)) * self.scale
        attention_probs = ops.softmax(attention_probs, dim=-1)
        attention_probs = self.attn_drop(attention_probs)

        context_layer = ops.transpose((attention_probs @ value), 1, 2).reshape(batch_size, hidden_size, num_channels)
