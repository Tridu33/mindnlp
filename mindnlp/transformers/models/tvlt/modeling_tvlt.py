...
class TvltModelOutput(ModelOutput):
    ...
    Args:
        ...
        last_pixel_hidden_state (`mindspore.Tensor` of shape `(batch_size, pixel_sequence_length, hidden_size)`):
            Pixel sequence of hidden-states at the output of the last layer of the model.
        last_audio_hidden_state (`mindspore.Tensor` of shape `(batch_size, audio_sequence_length, hidden_size)`):
            Audio sequence of hidden-states at the output of the last layer of the model.
        pixel_label_masks (`mindspore.Tensor` of shape `(batch_size, pixel_patch_length)`):
            Tensor indicating which pixel patches are masked (1) and which are not (0).
        audio_label_masks (`mindspore.Tensor` of shape `(batch_size, audio_patch_length)`):
            Tensor indicating which audio patches are masked (1) and which are not (0).
        pixel_ids_restore (`mindspore.Tensor` of shape `(batch_size, pixel_patch_length)`):
            Tensor containing the ids permutation of pixel masking.
        audio_ids_restore (`mindspore.Tensor` of shape `(batch_size, audio_patch_length)`):
    ...
    last_pixel_hidden_state: mindspore.Tensor = None
    last_audio_hidden_state: mindspore.Tensor = None
    pixel_label_masks: mindspore.Tensor = None
    audio_label_masks: mindspore.Tensor = None
    pixel_ids_restore: mindspore.Tensor = None
    audio_ids_restore: mindspore.Tensor = None
...
class TvltForPreTrainingOutput(ModelOutput):
    ...
    Args:
        ...
        matching_logits (`mindspore.Tensor` of shape `(batch_size, 1)`):
            Matching objective logits.
        pixel_logits (`mindspore.Tensor` of shape
            `(batch_size, pixel_patch_length, image_patch_size ** 3 * pixel_num_channels)`): Pixel reconstruction
            logits.
        audio_logits (`mindspore.Tensor` of shape
    ...
    matching_logits: mindspore.Tensor = None
    pixel_logits: mindspore.Tensor = None
    audio_logits: mindspore.Tensor = None
    hidden_states: Optional[Tuple[mindspore.Tensor, ...]] = None
    attentions: Optional[Tuple[mindspore.Tensor, ...]] = None
...
def generate_pixel_mask_noise(pixel_values, pixel_mask=None, mask_ratio=0.75):
    ...
    noise = ops.rand((batch_size, seq_len))  # noise in [0, 1]
...
def generate_audio_mask_noise(audio_values, audio_mask=None, mask_ratio=0.75, mask_type="patch-level", freq_len=8):
    ...
    if mask_type == "frame-level":
        ...
        # noise = (
        #     ops.rand(batch_size, num_time_patches)
        #     .unsqueeze(-1)
        #     .repeat(1, 1, freq_len)
...
def random_masking(sequence, noise, len_keep, attention_masks=None):
    ...
    # sequence_masked = ops.gather_elements(sequence, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, hidden_dim))
    sequence_masked = ops.gather(sequence, dim=1, index=ids_keep.unsqueeze(-1).tile((1, 1, hidden_dim)))

    # generate the binary mask: 0 is keep, 1 is remove
    label_masks = ops.ones(batch_size, seq_len)
    label_masks[:, :len_keep] = 0
    # unshuffle to get the binary mask
    label_masks = ops.gather(label_masks, dim=1, index=ids_restore)

    if attention_masks is not None:
        label_masks *= attention_masks
        attention_masks = ops.gather(attention_masks, dim=1, index=ids_keep)
...
class TvltPixelEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values, attention_masks=None):
        ...
        embeddings += self.pos_embed_v.tile((1, num_frames, 1))
        embeddings += ops.repeat_interleave(self.temporal_embed[:, :num_frames], self.num_patches_per_image, dim=1)
...
class TvltAudioEmbeddings(nn.Module):
    ...
    def forward(self, audio_values, attention_masks=None):
        ...
        num_time_patches = embeddings.shape[1] // self.num_freq_patches
        # embeddings += self.freq_embed.repeat(1, num_time_patches, 1)
        embeddings += self.freq_embed.tile((1, num_time_patches, 1))
        embeddings += ops.repeat_interleave(self.pos_embed_a[:, :num_time_patches], self.num_freq_patches, dim=1)
...
class TvltPixelPatchEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.projection = nn.Conv2d(num_channels, hidden_size, kernel_size=tuple(patch_size), stride=tuple(patch_size),bias=True)
...
class TvltAudioPatchEmbeddings(nn.Module):
    ...
    def forward(self, audio_values: mindspore.Tensor) -> mindspore.Tensor:
...
class TvltModel(TvltPreTrainedModel):
    ...
    def forward(
        ...
        audio_values: mindspore.Tensor,
        pixel_mask: Optional[mindspore.Tensor] = None,
        audio_mask: Optional[mindspore.Tensor] = None,
        mask_pixel: bool = False,
        mask_audio: bool = False,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[mindspore.Tensor], TvltModelOutput]:
        r"""
        Returns:
        ...
        # embedding_output = ops.cat(
        #     [self.cls_embedding.repeat(batch_size, 1, 1), pixel_embedding_output, audio_embedding_output], 1
        # )
        embedding_output = ops.cat(
            [self.cls_embedding.tile((batch_size, 1, 1)), pixel_embedding_output, audio_embedding_output], 1
        )
        masked_pixel_len = pixel_embedding_output.shape[1]

        attention_mask = None
        if pixel_mask is not None and audio_mask is not None:
            # attention_mask = ops.cat([pixel_mask[:, :1], pixel_mask, audio_mask], 1)
            attention_mask = ops.cat([pixel_mask[:, :1], pixel_mask, audio_mask.to(pixel_mask.dtype)], 1)
...
class TvltForPreTraining(TvltPreTrainedModel):
    ...
    def patchify_pixel(self, pixel_values):
        ...
        patchified_pixel_values = ops.einsum("ntchpwq->nthwpqc", patchified_pixel_values)
    ...
    def patchify_audio(self, audio_values):
        ...
        patchified_audio_values = ops.einsum("nchpwq->nhwpqc", patchified_audio_values)
    ...
    def pixel_mae_loss(self, pixel_values, pixel_predictions, mask):
        ...
        loss = loss.mean(axis=-1)  # [batch_size, pixel_pixel_length], mean loss per patch
    ...
    def audio_mae_loss(self, audio_values, audio_predictions, mask):
        ...
        loss = loss.mean(axis=-1)  # [batch_size, audio_pixel_length], mean loss per patch
        loss = (loss * mask).sum() / mask.sum()  # mean loss on removed patches
        return loss
    ...
    def concatenate_mask(self, mask_token, sequence, ids_restore):
        ...
        mask_tokens = mask_token.tile((batch_size, ids_restore.shape[1] - seq_length, 1))
        padded_sequence = ops.cat([sequence, mask_tokens], dim=1)
        padded_sequence = ops.gather(
            padded_sequence, dim=1, index=ids_restore.unsqueeze(-1).tile((1, 1, dim))
        )  # unshuffle
    ...
    def forward(
        ...
        pixel_values_mixed: Optional[mindspore.Tensor] = None,
        pixel_mask_mixed: Optional[mindspore.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[mindspore.Tensor], TvltForPreTrainingOutput]:
        r"""
        pixel_values_mixed (`mindspore.Tensor` of shape `(batch_size, num_frames, num_channels, height, width)`):
        ...
        pixel_mask_mixed (`mindspore.Tensor` of shape `(batch_size, num_channels, height, width)`):
        ...
        labels (`mindspore.Tensor` of shape `(batch_size, num_labels)`, *optional*):
        ...
        if self.task_matching:
            ...
            loss = F.binary_cross_entropy_with_logits(matching_logits.view(-1), labels.view(-1))
            total_loss += loss
        ...
        if self.task_mae and self.training:
            ...
            pixel_decoder_input = pixel_decoder_input + self.decoder_pixel_pos_embed.tile((1, num_frames, 1))
            pixel_decoder_input = pixel_decoder_input + ops.repeat_interleave(
            ...
            num_time_patches = audio_decoder_input.shape[1] // self.num_freq_patches
            # audio_decoder_input = audio_decoder_input + self.decoder_freq_embed.repeat(1, num_time_patches, 1)
            audio_decoder_input = audio_decoder_input + self.decoder_freq_embed.tile((1, num_time_patches, 1))
            audio_decoder_input = audio_decoder_input + ops.repeat_interleave(
