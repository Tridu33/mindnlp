...
class ViTMAEModelOutput(ModelOutput):
    ...
    ids_restore: mindspore.Tensor = None
...
def get_2d_sincos_pos_embed(embed_dim, grid_size, add_cls_token=False):
    ...
    Returns:
        (`mindspore.Tensor` of shape (grid_size*grid_size, embed_dim) or (1+grid_size*grid_size, embed_dim): the
...
class ViTMAEEmbeddings(nn.Module):
    ...
    def random_masking(self, sequence, noise=None):
        ...
        Args:
            sequence (`mindspore.Tensor` of shape `(batch_size, sequence_length, dim)`)
            noise (`mindspore.Tensor` of shape `(batch_size, sequence_length)`, *optional*) which is
...
class ViTMAEForPreTraining(ViTMAEPreTrainedModel):
    ...
    def unpatchify(self, patchified_pixel_values, original_image_size: Optional[Tuple[int, int]] = None):
        ...
        patchified_pixel_values = ops.einsum("nhwpqc->nchpwq", patchified_pixel_values)
    ...
    def forward_loss(self, pixel_values, pred, mask, interpolate_pos_encoding: bool = False):
        ...
        Args:
            pixel_values (`mindspore.Tensor` of shape `(batch_size, num_channels, height, width)`):
                Pixel values.
            pred (`mindspore.Tensor` of shape `(batch_size, num_patches, patch_size**2 * num_channels)`:
