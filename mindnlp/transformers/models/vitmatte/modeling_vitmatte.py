...
class ImageMattingOutput(ModelOutput):
    ...
    Args:
        loss (`mindspore.Tensor` of shape `(1,)`, *optional*, returned when `labels` is provided):
            Loss.
        alphas (`mindspore.Tensor` of shape `(batch_size, num_channels, height, width)`):
...
class VitMatteFusionBlock(nn.Module):
    ...
    def forward(self, features, detailed_feature_map):
        upscaled_features = nn.functional.interpolate(features, scale_factor=2., mode="bilinear", align_corners=False, recompute_scale_factor=True)
        out = ops.cat([detailed_feature_map, upscaled_features], dim=1)
        out = self.conv(out)
...
class VitMatteDetailCaptureModule(nn.Module):
    ...
    def forward(self, features, pixel_values):
        ...
        alphas = ops.sigmoid(self.matting_head(features))
