...
from mindnlp.core.nn import CrossEntropyLoss, MSELoss
...
class VivitTubeletEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.projection = nn.Conv3d(
            config.num_channels, config.hidden_size, kernel_size=tuple(config.tubelet_size), stride=tuple(config.tubelet_size)
        )
...
class VivitEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.position_embeddings = nn.Parameter(
            ops.zeros(1, self.patch_embeddings.num_patches + 1, config.hidden_size)
        )
...
class VivitForVideoClassification(VivitPreTrainedModel):
    ...
    def forward(
        ...
        interpolate_pos_encoding: bool = False,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[mindspore.Tensor], ImageClassifierOutput]:
        r"""
