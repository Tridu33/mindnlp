...
def _median_filter(inputs: mindspore.Tensor, filter_width: int) -> mindspore.Tensor:
    ...
    # sort() is faster than ops.median (https://github.com/pytorch/pytorch/issues/51450)
    result = ops.unfold(inputs, -1, filter_width, 1).sort()[0][..., pad_width]
    return result
...
def _pad_to_max_length(
    ...
    for current_segment_list in current_segments:
        if current_segment_list is not None and len([d["tokens"] for d in current_segment_list]) > 0:
            sequence = ops.cat([d["tokens"] for d in current_segment_list], dim=-1)

            if cut_off_length is not None:
                sequence = sequence[-cut_off_length:]

            if bos_token_tensor is not None:
                sequence = ops.cat([bos_token_tensor, sequence])
    ...
    sequences = ops.stack(sequences, dim=0)
    return sequences
...
class WhisperGenerationMixin:
    def _extract_token_timestamps(self, generate_outputs, alignment_heads, time_precision=0.02, num_frames=None):
        ...
        if "beam_indices" in generate_outputs:
            ...
            weights = ops.stack(
                [
                    ops.index_select(weights[:, :, i, :], dim=0, index=beam_indices[:, i])
        ...
        timestamps = ops.zeros_like(generate_outputs.sequences, dtype=mindspore.float32)[:, : input_length + 1]
        ...
        if num_frames is not None:
            ...
            elif isinstance(num_frames, (mindspore.Tensor)) and len(ops.unique(num_frames)) == 1:
                weights = weights[..., : num_frames[0] // 2]
        ...
        if num_frames is None or isinstance(num_frames, int):
            ...
            std = ops.std(weights, dim=-2, keepdim=True, correction=0)
            mean = ops.mean(weights, dim=-2, keepdim=True)
        ...
        for batch_idx in range(batch_size):
            if num_frames is not None and isinstance(num_frames, (tuple, list, np.ndarray, mindspore.Tensor)):
                ...
                # Normalize and smoothen the weights.
                std = ops.std(matrix, dim=-2, keepdim=True, correction=0)
                mean = ops.mean(matrix, dim=-2, keepdim=True)
    ...
    def generate(
        ...
        **kwargs,
        ...
        Return:
            [`~utils.ModelOutput`] or `mindspore.Tensor` or `Dict[str, Any]`: A [`~utils.ModelOutput`] (if `return_dict_in_generate=True`
        ...
        sequences = _pad_to_max_length(
            final_segments, generation_config.pad_token_id, padding_side="right"
        )
        ...
        if is_shortform:
            ...
            if generation_config.max_new_tokens is None and generation_config.max_length is None:
                eos_tokens = ops.full((sequences.shape[0], 1), generation_config.eos_token_id)
                sequences = ops.cat([sequences, eos_tokens], dim=-1)

            if return_token_timestamps:
                outputs = {}
                outputs["sequences"] = sequences
                outputs["token_timestamps"] = ops.stack([d["token_timestamps"] for d in seek_outputs], dim=0)
            else:
                outputs = sequences

            if return_dict_in_generate and generation_config.return_dict_in_generate:
                dict_outputs = self._stack_split_outputs(seek_outputs, model_output_type, kwargs)
    ...
    def generate_with_fallback(
        ...
        for fallback_idx, temperature in enumerate(temperatures):
            ...
            decoder_input_ids = ops.stack(new_decoder_input_ids)
            segment_input = ops.stack(new_segment_input)
            if "decoder_attention_mask" in kwargs:
                kwargs["decoder_attention_mask"] = ops.stack(new_decoder_attention_mask)
    ...
    def _postprocess_outputs(
        ...
        start_idx = decoder_input_ids.shape[-1] if not is_shortform else mindspore.tensor(0)
            ...
            if key in ["decoder_attentions", "decoder_hidden_states", "cross_attentions"]:
                ...
                if not is_shortform:
                    ...
                    for layer_idx in range(self.config.decoder_layers):
                        ...
                        for cache_cls in [values.self_attention_cache, values.cross_attention_cache]:
                            for v in [cache_cls.key_cache, cache_cls.value_cache]:
                                layer_past_key_values.append(v[layer_idx][batch_idx][None])
                    ...
                    for v in range(len(values)):
                        layer_past_key_values = []
                        for w in values[v]:
                            layer_past_key_values.append(w[batch_idx][None])
                        all_past_key_values.append(tuple(layer_past_key_values))
                    return tuple(all_past_key_values)
    ...
    def _stack_split_outputs(self, seek_outputs, model_output_type, kwargs):
        # Stack back seek_outputs tensors after splitting them with the split_by_batch_index method
        outputs = {}
        for key in seek_outputs[0].keys():
            if key == "sequences":
                outputs[key] = ops.stack([v[key] for v in seek_outputs], dim=0)
            if key in ["scores", "encoder_attentions", "encoder_hidden_states", "logits"]:
                outputs[key] = tuple(
                    ops.stack([v[key][i] for v in seek_outputs]) for i in range(len(seek_outputs[0][key]))
                )
            if key in ["decoder_attentions", "decoder_hidden_states", "cross_attentions"]:
                outputs[key] = tuple(
                    tuple(
                        ops.stack([v[key][i][j] for v in seek_outputs]).squeeze(1)
    ...
    def _expand_variables_for_generation(
        ...
        if generation_config.num_return_sequences is not None and generation_config.num_return_sequences > 1:
            ...
            input_features = ops.repeat_interleave(input_features, generation_config.num_return_sequences, dim=0)
            seek = ops.repeat_interleave(seek, generation_config.num_return_sequences, dim=0)
            max_frames = ops.repeat_interleave(max_frames, generation_config.num_return_sequences, dim=0)
            init_tokens = ops.repeat_interleave(init_tokens, generation_config.num_return_sequences, dim=0)
    ...
    def _setup_no_speech_detection(logits_processor, segment_input, decoder_input_ids, kwargs):
        ...
        extra_kwargs = {k: v for k, v in kwargs.items() if ops.is_tensor(v)}
    ...
    def _retrieve_init_tokens(self, input_features, batch_size, generation_config, config, num_segment_frames, kwargs):
        ...
        return ops.as_tensor(init_tokens, dtype=mindspore.int64).broadcast_to((batch_size, -1))
    ...
    def detect_language(
        ...
        Return:
            A `mindspore.Tensor` representing the detected language ids.
        """
        ...
        decoder_input_ids = (
            ops.ones((batch_size, 1), dtype=mindspore.int64)
            * generation_config.decoder_start_token_id
        )

        with no_grad():
            logits = self(**inputs, decoder_input_ids=decoder_input_ids).logits[:, -1] # pylint: disable=not-callable

        non_lang_mask = ops.ones_like(logits[0], dtype=mindspore.bool_)
    ...
    def _retrieve_max_frames_and_seek(batch_size, attention_mask, total_input_frames, is_shortform):
        if batch_size > 1 and not is_shortform and attention_mask is None:
            ...
            seek = ops.zeros((batch_size,), dtype=mindspore.int64)
        else:
            max_frames = ops.ones((batch_size,), dtype=mindspore.int64) * total_input_frames
            seek = ops.zeros((batch_size,), dtype=mindspore.int64)

        return max_frames, seek

    def _retrieve_logit_processors(self, generation_config, logits_processor, begin_index, num_beams):
    ...
    def _maybe_reduce_batch(input_features, seek, max_frames, cur_bsz, batch_idx_map):
        ...
        for i in range(prev_bsz):
            ...
            if seek[prev_i] >= max_frames[prev_i]:
                ...
                cur_bsz -= 1
                input_features = ops.cat([input_features[:cut_index], input_features[cut_index + 1 :]], dim=0)
            else:
    ...
    def _get_input_segment(input_features, seek, seek_num_frames, num_segment_frames, cur_bsz, batch_idx_map):
        ...
        segment_input = ops.cat(segment_input, dim=0)
    ...
    def _prepare_decoder_input_ids(
        ...
        if any(do_condition_on_prev_tokens) and len(current_segments[0]) > 0:
            ...
            if prompt_ids is not None and generation_config.prompt_condition_type == "all-segments":
                ...
                one_tensor = ops.ones((cur_bsz, 1), dtype=mindspore.int64)
            ...
            decoder_input_ids = ops.cat([prev_tokens, decoder_input_ids], dim=-1)

            kwargs["decoder_attention_mask"] = decoder_input_ids != generation_config.pad_token_id
        elif prompt_ids is not None:
            prev_tokens = prompt_ids[None].tile((decoder_input_ids.shape[0], 1))
            decoder_input_ids = ops.cat([prev_tokens, decoder_input_ids], dim=-1)
    ...
    def _retrieve_segment(
        ...
        timestamp_segment_indices = ops.nonzero(timestamp_tokens[:-1].int() & timestamp_tokens[1:].int(), as_tuple=True)[0]
        timestamp_segment_indices = timestamp_segment_indices.add(1)
