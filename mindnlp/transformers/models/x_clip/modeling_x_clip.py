...
def x_clip_loss(similarity: mindspore.Tensor) -> mindspore.Tensor:
...
class XCLIPOutput(ModelOutput):
    ...
    Args:
        ...
        logits_per_video (`mindspore.Tensor` of shape `(video_batch_size, text_batch_size)`):
            The scaled dot product scores between `video_embeds` and `text_embeds`. This represents the video-text
            similarity scores.
        logits_per_text (`mindspore.Tensor` of shape `(text_batch_size, video_batch_size)`):
        ...
        video_embeds(`mindspore.Tensor` of shape `(batch_size, output_dim`):
    ...
    logits_per_video: mindspore.Tensor = None
    logits_per_text: mindspore.Tensor = None
    text_embeds: mindspore.Tensor = None
    video_embeds: mindspore.Tensor = None
...
class XCLIPTextTransformer(nn.Module):
    ...
    def forward(
        ...
        pooled_output = last_hidden_state[ops.arange(last_hidden_state.shape[0]), ops.argmax(input_ids, dim=-1)]
...
class XCLIPMultiframeIntegrationTransformer(nn.Module):
    ...
    def __init__(self, config: XCLIPVisionConfig):
        ...
        self.position_embedding = nn.Parameter(ops.empty(1, config.num_frames, config.hidden_size))
        self.encoder = XCLIPEncoder(config)
    ...
    def forward(
        ...
        pooled_output = ops.mean(last_hidden_state, dim=1, keepdim=False)
...
class XCLIPCrossAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        attn = (queries @ ops.transpose(keys, -2, -1)) * self.scale
        attn = ops.softmax(attn, dim=-1)
        attn = self.attn_drop(attn)

        x = ops.transpose((attn @ values), 1, 2).reshape(batch_size, query_seq_len, hidden_size)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class XCLIPPromptGenerator(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.alpha = nn.Parameter(ops.ones(embed_dim) * config.prompt_alpha)
...
class XCLIPModel(XCLIPPreTrainedModel):
    ...
    def __init__(self, config: XCLIPConfig):
        ...
        self.prompts_visual_projection = nn.Parameter(ops.randn(self.vision_embed_dim, self.projection_dim))
    ...
    def get_video_features(
        ...
        r"""
        Returns:
            video_features (`mindspore.Tensor` of shape `(batch_size, output_dim`): The video embeddings obtained by
    ...
    def forward(
        ...
        img_features = ops.mean(img_features, dim=1, keepdim=False)
        ...
        text_embeds = text_embeds.unsqueeze(0).broadcast_to((batch_size, -1, -1))
        text_embeds = text_embeds + self.prompts_generator(text_embeds, img_features)

        # normalized features
        video_embeds = video_embeds / ops.norm(video_embeds, p=2, dim=-1, keepdim=True)
        text_embeds = text_embeds / ops.norm(text_embeds, p=2, dim=-1, keepdim=True)

        # cosine similarity as logits
        logit_scale = self.logit_scale.exp()
        logits_per_video = ops.einsum("bd,bkd->bk", video_embeds, logit_scale * text_embeds)
