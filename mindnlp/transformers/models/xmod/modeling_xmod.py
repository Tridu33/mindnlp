...
class XmodOutput(nn.Module):
    ...
    def lang_adapter(self, lang_ids: Tensor, hidden_states: Tensor):
        # Process subsequent samples with the same lang_id in parallel
        lang_ids, lang_lengths = ops.unique_consecutive(lang_ids, return_counts=True)
        ...
        split_hidden_states = ops.split(hidden_states, lang_lengths.tolist(), 0)
        lang_wise_outputs = []
        ...
        hidden_states = ops.cat(lang_wise_outputs, 0)
...
class XmodPreTrainedModel(PreTrainedModel):
    ...
    def freeze_embeddings_and_language_adapters(self):
        ...
        for parameter in self.roberta.embeddings.get_parameters():
            parameter.requires_grad = False
        logger.info("Freezing adapters")
        for layer in self.roberta.encoder.layer:
            if layer.output.adapter_layer_norm is not None:
                for parameter in layer.output.adapter_layer_norm.get_parameters():
                    parameter.requires_grad = False
            for parameter in layer.output.adapter_modules.get_parameters():
                parameter.requires_grad = False
...
class XmodModel(XmodPreTrainedModel):
    ...
    def forward(
        ...
        past_key_values (`tuple(tuple(Tensor))` of length `config.n_layers` with each tuple having 4 tensors:
        ...
        if lang_ids is None:
            ...
            lang_ids = default_lang_id * ops.ones(batch_size)
...
class XmodForCausalLM(XmodPreTrainedModel):
    ...
    def forward(
        ...
        Returns: `transformers.modeling_outputs.CausalLMOutputWithCrossAttentions` or `tuple(Tensor)`
...
class XmodForMultipleChoice(XmodPreTrainedModel):
    ...
    def forward(
        ...
        flat_lang_ids = lang_ids.repeat(input_ids.shape[0] * input_ids.shape[-1]) if lang_ids is not None else None
